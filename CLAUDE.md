...
| **Phase F complete** → owner re-playtest. Then Phase G (29 stories + SessionMap — `Level screen.png` template). Still open from feedback: narration audio (edge-tts per docs — dev-machine generation OK, runtime TTS forbidden), teacher avatar real art, race pickup icons (mockup 17 shows emoji-style icons on cards), strafe-lean on lane switch (KI clips ready) | ⬜ next |
