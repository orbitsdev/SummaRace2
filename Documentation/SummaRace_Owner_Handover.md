...
| **`assets_credits.md`** (repo root, not `Documentation/`) | The GDD §6.8 credits tracker, built from what is actually in the repo. Every third-party pack, font, sound and character with its author, source and licence *as evidenced by a file on disk* — plus a 21-entry **UNVERIFIED** register of what could not be evidenced. Its §1a lists three things to settle before the APK ships: **this GitHub repo is public** and holds the raw Asset Store packs, **two shipped audio files have no traceable source**, and **the 160 narration clips have no licence grant**. | Before you distribute to 40 devices, and before anyone asks where the art came from. |
